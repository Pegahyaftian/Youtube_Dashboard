...
@st.cache_data
def load_data():
    df_agg = pd.read_csv('dataset/Aggregated_Metrics_By_Video.csv').iloc[1:,:]
    ...
    df_agg_sub = pd.read_csv('dataset/Aggregated_Metrics_By_Country_And_Subscriber_Status.csv')
    df_comments = pd.read_csv('dataset/Aggregated_Metrics_By_Video.csv')
    df_time = pd.read_csv('dataset/Video_Performance_Over_Time.csv')
    df_time['Date'] = pd.to_datetime(df_time['Date'])
    print(df_time)
    return df_agg, df_agg_sub, df_comments, df_time

#create dataframes fromm the fundtions
df_agg, df_agg_sub, df_comments, df_time = load_data()

df_agg_diff = df_agg.copy()
metric_date_12mo = df_agg_diff['Video publish time'].max() - pd.DateOffset(months=12)
metric_medians12mo = df_agg_diff[df_agg_diff['Video publish time'] >= metric_date_12mo].median()

#create differences from the median for values
#Just numeric columns
numeric_cols = np.array((df_agg_diff.dtypes == 'float64') | (df_agg_diff.dtypes == 'int64')) 
df_agg_diff.iloc[:,numeric_cols] = (df_agg_diff.iloc[:,numeric_cols]- metric_medians12mo).div(metric_medians12mo)
...
df_time_diff_yr = df_time_diff[df_time_diff['Video publish time'] >= metric_date_12mo]\
...
#################################################
#build dashboard and Streamlit App
#################################################
